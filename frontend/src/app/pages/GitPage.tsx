...
function RepoSection({
    ...
    [
      {
        ...
        scope: "/git",
        handler: () => {
          if (commitMessageRef.current.trim()) handleCommitRef.current();
        },
      },
      {
  ...
  const discardDescription = discardPending ? (
    discardPending.length === 1 ? (
      <>
        Discard changes to{" "}
        <span className="text-foreground font-semibold">{discardPending[0].split("/").pop()}</span>?
        This cannot be undone.
      </>
    ) : (
      <>
        Discard changes to{" "}
        <span className="text-foreground font-semibold">{discardPending.length} files</span>? This
        cannot be undone.
      </>
    )
  ) : (
  ...
  return (
    <div className="flex flex-col">
      <ConfirmDialog
        ...
        onOpenChange={(o) => {
          if (!o && !discarding) setDiscardPending(null);
        }}
      ...
      <ConfirmDialog
        ...
        onOpenChange={(o) => {
          if (!o && !checkingOut) setCheckoutPending(null);
        }}
        title="Uncommitted changes"
        description={
          <>
            You have uncommitted changes. Switching to{" "}
            <span className="text-foreground font-semibold">{checkoutPending}</span> may fail or
      ...
      {showConflictEditor && conflictFilePaths.length > 0 ? (
        <GitConflictEditor
          ...
          onComplete={() => {
            ...
            setConflictEditorInitialFile(null);
            setSyncResult(null);
            refresh();
          }}
          onAbort={() => {
...
export function GitPage() {
  ...
  const buildSelectedCommitFile =
    selectedCommitFile?.repoPath === buildPath
      ? { hash: selectedCommitFile.hash, path: selectedCommitFile.path }
      : null;
  const defineSelectedCommitFile =
    selectedCommitFile?.repoPath === definePath
      ? { hash: selectedCommitFile.hash, path: selectedCommitFile.path }
      : null;
              ...
              <div className="flex flex-col h-full overflow-y-auto divide-y divide-foreground/10">
                {buildPath && (
                  <RepoSection
                    ...
                    onBranchesUpdated={(b, branch, ahead) => {
                      setBuildBranches(b);
                      setBuildCurrentBranch(branch);
                      setBuildAhead(ahead);
                    }}
                ...
                {definePath && (
                  <RepoSection
                    ...
                    onBranchesUpdated={(b, branch, ahead) => {
                      setDefineBranches(b);
                      setDefineCurrentBranch(branch);
                      setDefineAhead(ahead);
                    }}
                    isSuggestionEnabled={isSuggestionEnabled}
                    addSuggestion={addSuggestion}
                    localAIEnabled={localAIState.enabled}
                    ...
                    <div className="flex-1 overflow-hidden">
                      <GitGraph
                        ...
                        onSelectCommitFile={(hash, path) =>
                          handleSelectCommitFile(activeGraphRepoPath || buildPath, hash, path)
                        }
                      />
                    </div>
                  </div>
