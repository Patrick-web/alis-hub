...
const stages: Array<{
...
}> = [
  {
    ...
    description:
      "Understand how Skills are packaged, discovered, and synchronised into the deployment registry.",
...
const suggestedPrompts = [
  {
    ...
    prompt:
      "Use the Alis Build MCP and the deployed Skills registry to add the relevant skills to my agent. Start by listing available skills, then recommend which ones should be enabled for this agent.",
  },
  {
    title: "Add a skill to an agent project",
    prompt:
      "Use the Alis Build MCP guidance to add a skill to this agent project. Inspect the agent structure, identify where the skill should be wired in, and make the minimal code changes.",
  },
  {
    title: "Add skills to an MCP server",
    prompt:
      "Use the dedicated Alis Build skill from the Skills registry to add Skills support to my MCP server. Guide me through connecting the MCP server to the deployed Skills service and exposing dynamic discovery and loading tools.",
  },
  {
    title: "Create a new skill",
    prompt:
      "Help me create a new skill package for this workflow. Generate a SKILL.md with clear metadata and instructions, suggest useful references or scripts, and prepare it for the skills repository sync.",
  },
];
    ...
    <PageLayout
      ...
      actions={
        <div className="flex items-center gap-[6px]">
          <Button
            variant="secondary"
            onClick={() => Browser.OpenURL(SKILLS_REPO_URL)}
      ...
      <div className="px-[24px] py-[20px] max-w-[900px] mx-auto w-full">
        <div className="flex flex-col gap-[16px]">
          <p className="text-[12px] text-foreground/70 leading-[1.6]">
            The Skills service packages reusable Alis Build workflows as structured skills —
            discoverable by coding agents via MCP. Skills provide step-by-step guidance that agents
            follow to implement Alis Build patterns.
          </p>
          ...
          {selectedStage === "overview" && (
            <StageCard
              ...
              icon={<Icon icon="solar:layers-linear" className="text-brand" />}
            >
              <p className="text-[11px] text-foreground/60 mb-[12px] leading-[1.5]">
                Skills are structured SKILL.md files stored in a Git repository. The Skills service
                syncs this repository into a registry that coding agents can query via MCP to
                ...
                {[
                  {
                    ...
                    title: "Skills are SKILL.md files",
                    desc: "Each skill is a markdown file with metadata, step-by-step instructions, and optional scripts or references.",
                  },
                  {
                    icon: "solar:refresh-linear",
                    title: "Synced from a Git repo",
                    desc: "The Skills service watches a Git repository and syncs skills into a queryable registry.",
                  },
                  {
                    icon: "solar:server-minimalistic-linear",
                    title: "Exposed via MCP",
                    desc: "Agents use the Skills MCP tools to list, load, and follow skill instructions at runtime.",
                  },
                ].map((item) => (
                  <div
                    key={item.title}
                    className="flex items-start gap-[10px] px-[12px] py-[10px] bg-muted border border-border rounded-[4px]"
                  >
              ...
              <div className="mt-[12px] flex items-center gap-[8px]">
                ...
                <Button
                  variant="ghost"
                  onClick={() => setSelectedStage("prerequisites")}
          ...
          {selectedStage === "prerequisites" && (
            <StageCard
              step={2}
              title="Prerequisites"
              subtitle="Confirm the product and service for your Skills deployment"
          ...
          {selectedStage === "service" && (
            <StageCard
              step={3}
              title="Skills Service"
              subtitle="Install, define, build, and deploy the Skills codeblock"
              icon={<Icon icon="solar:book-2-linear" className="text-brand" />}
            >
              <p className="text-[11px] text-foreground/60 mb-[12px] leading-[1.5]">
                Install the Skills codeblock into your service via the VS Code extension, then use
                the standard Alis Build workflow to define, build, and deploy.
              </p>
              <div className="flex flex-col gap-[8px] mb-[14px]">
                {[
                  {
                    n: 1,
                    label: "Install Skills codeblock",
                    desc: "Add the Skills codeblock to your service via the VS Code extension.",
                  },
                  {
                    n: 2,
                    label: "Configure repository URL",
                    desc: "Set the SKILL_REPOSITORY_URL environment variable to point to your skills Git repo.",
                  },
                  {
                    n: 3,
                    label: "Build & Deploy",
                    desc: "Build the service image and deploy it to your environment.",
                  },
                ].map((item) => (
          ...
          {selectedStage === "next" && (
            <StageCard
              step={4}
              title="Next Steps"
              subtitle="Suggested agent prompts for using your Skills service"
              icon={<Icon icon="solar:lightbulb-linear" className="text-brand" />}
            >
              <p className="text-[11px] text-foreground/60 mb-[12px] leading-[1.5]">
                Once deployed, use these prompts with your coding agent (Claude Code, Gemini CLI,
                etc.) to leverage the Skills registry:
              </p>
              <div className="flex flex-col gap-[8px]">
                {suggestedPrompts.map((p) => (
                  <div
                    key={p.title}
                    className="p-[12px] bg-muted border border-border rounded-[4px]"
                  >
                    <div className="flex items-center justify-between mb-[6px]">
                      ...
                      >
                        <Icon
                          icon={copied === p.title ? "solar:check-linear" : "solar:copy-linear"}
                          className="text-[11px]"
                        />
