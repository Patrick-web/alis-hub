      ...
      <div className="px-[24px] py-[20px] max-w-[900px] mx-auto w-full">
        <div className="flex flex-col gap-[16px]">
          <p className="text-[12px] text-foreground/70 leading-[1.6]">
            The Identity flow guides you through deploying and configuring an Identity service for
            your product. It handles OAuth authentication, session management, and application
            integrations for agents and enterprise workflows.
          </p>
          ...
          >
            <p className="text-[11px] text-foreground/60 mb-[12px] leading-[1.5]">
              The Identity service is deployed as a standard Alis Build service. Install the
              Identity codeblock via the VS Code extension, then build and deploy it.
            </p>
            <div className="flex flex-col gap-[8px] mb-[14px]">
              {[
                {
                  label: "SUPER_ADMINS",
                  desc: "Comma-separated list of super admin email addresses.",
                },
                {
                  label: "IDENTITY_SERVICE_URL",
                  desc: "Public URL of the deployed Identity service (e.g. https://users-v1-{project}.{region}.run.app).",
                },
                {
                  label: "COOKIE_DOMAIN",
                  desc: "Base domain for session cookies (e.g. .{your-launchpad-domain}).",
                },
              ].map((env) => (
                <div
                  key={env.label}
                ...
                >
                  <p className="text-[11px] font-bold text-foreground font-mono mb-[2px]">
                    {env.label}
                  </p>
          ...
          <StageCard
            ...
            action={
              <Button
                variant="secondary"
                onClick={() => Browser.OpenURL(GOOGLE_OAUTH_URL)}
          ...
          >
            <p className="text-[11px] text-foreground/60 mb-[12px] leading-[1.5]">
              Create a <strong className="text-foreground">Web application</strong> OAuth 2.0 client
              in the Google Cloud Console. Set the following authorised redirect URIs:
            </p>
            <div className="flex flex-col gap-[4px]">
              {[
                ...
                <div
                  key={uri}
                  className="flex items-center gap-[8px] px-[10px] py-[6px] bg-muted border border-border rounded-[4px]"
                >
                  <Icon
                    icon="solar:link-linear"
                    className="text-foreground/35 text-[12px] shrink-0"
                  />
          ...
          <StageCard
            ...
            action={
              <Button
                variant="secondary"
                onClick={() => Browser.OpenURL(MICROSOFT_OAUTH_URL)}
                icon={<Icon icon="solar:arrow-right-up-linear" className="text-sm" />}
              >
                Azure Portal
              </Button>
          ...
          >
            <p className="text-[11px] text-foreground/60 mb-[12px] leading-[1.5]">
              Register a new application in Microsoft Entra ID (Azure AD) with the following
              settings:
            </p>
              ...
              {[
                {
                  ...
                  value: "Any Entra ID Tenant + Personal Microsoft accounts",
                },
                { label: "Redirect platform", value: "Web" },
                {
                  label: "Redirect URI",
                  value: "https://{identity-service-url}/callback/microsoft",
                },
                { label: "Local redirect URI", value: "http://localhost:8080/callback/microsoft" },
              ].map((item) => (
                <div
                  key={item.label}
                  className="flex items-start gap-[10px] px-[12px] py-[8px] bg-muted border border-border rounded-[4px]"
                >
                  <span className="text-[10px] text-foreground/45 font-mono w-[160px] shrink-0">
                    {item.label}
                  </span>
          ...
          <div className="flex items-start gap-[10px] px-[14px] py-[12px] bg-brand-fill/6 border border-brand-fill/20 rounded-[4px]">
            ...
            <p className="text-[11px] text-foreground/60 leading-[1.5]">
              Full Identity setup with automated service selection is available in the VS Code
              extension. Use the Alis Build extension for the complete interactive workflow.
            </p>
          </div>
        </div>
      </div>
