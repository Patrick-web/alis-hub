        ...
        <div key={s.label} className="flex items-center">
          <div className="flex flex-col items-center gap-[6px] px-[14px] py-[10px] bg-card border border-border rounded-[4px]">
            <div className="flex items-center gap-[6px]">
              <div className="size-[18px] rounded-full bg-brand-fill/15 border border-brand-fill flex items-center justify-center shrink-0">
                <span className="text-[8px] font-bold text-brand font-mono">{s.n}</span>
              </div>
              <Icon icon={s.icon} className="text-brand text-[14px]" />
            </div>
            <span className="text-[9px] font-bold text-foreground font-mono uppercase">
              {s.label}
            </span>
          </div>
          {i < stages.length - 1 && (
            <Icon
              ...
              className="text-foreground/20 text-[16px] mx-[6px]"
            />
          )}
        </div>
      ))}
      ...
      <div className="px-[24px] py-[20px] max-w-[900px] mx-auto w-full">
        <div className="flex flex-col gap-[16px]">
          <p className="text-[12px] text-foreground/70 leading-[1.6]">
            Unlock the full potential of your AI Agent by transforming it into a versatile operator
            capable of interacting with the world. By giving it Tools, you equip your Agent with
            callable methods that it can autonomously invoke to perform complex actions, retrieve
            real-time data, or interact with external systems.
          </p>

          <div>
            <p className="text-[11px] font-bold text-foreground uppercase font-mono mb-[8px]">
              The 4-Stage Process
            </p>
            <AddToolDiagram />
          </div>
          ...
          >
            <p className="text-[11px] text-foreground/60 mb-[12px] leading-[1.6]">
              Note that a Tool is <strong>not built within the Agent service</strong>, but rather
              built in its own service independently from the Agent, and then registered with the
              Agent only once deployed.
            </p>
            <div className="flex flex-col gap-[8px] mb-[16px]">
              <div className="flex gap-[10px]">
                ...
                <div>
                  <p className="text-[11px] font-bold text-foreground">
                    Confirm/Configure the Host Service
                  </p>
                  <p className="text-[10px] text-foreground/50">
                    Create a new service with boilerplate code or select an existing one to host
                    your tool's logic.
                  </p>
                </div>
              </div>
              <div className="flex gap-[10px]">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-foreground/50">
                    Your local file system is automatically updated with the necessary services and
                    code.
                  </p>
                </div>
              </div>
          ...
          >
            <p className="text-[11px] text-foreground/60 mb-[12px] leading-[1.6]">
              Every powerful tool begins with a clear definition. You precisely define the tool's
              interface and capabilities in your Protocol Buffer (.proto) file. Specify what data
              the tool requires (inputs) and the structure of the information it will return
              (outputs). Embed rich, descriptive comments — these are vital instructions that
              explain the tool's purpose to the Agent.
            </p>
          ...
          >
            <p className="text-[11px] text-foreground/60 mb-[12px] leading-[1.6]">
              With the tool's interface defined, the next step is to write the actual code that
              performs its designated task. This involves developing the backend logic in your
              chosen server-side language (e.g., Go).
            </p>
            <p className="text-[10px] font-bold text-foreground/40 uppercase font-mono mb-[4px]">
              Example
            </p>
          ...
          >
            <p className="text-[11px] text-foreground/60 mb-[12px] leading-[1.6]">
              Once the tool's logic is fully implemented and tested, it's ready for deployment. This
              involves packaging your code as a service and deploying it to your chosen cloud
              environment, ensuring your tool is always available and scalable.
            </p>
            <p className="text-[10px] font-bold text-foreground/40 uppercase font-mono mb-[4px]">
              Example
            </p>
          ...
          >
            <p className="text-[11px] text-foreground/60 mb-[12px] leading-[1.6]">
              The final step connects your new, deployed tool directly to your Agent. You register
              this new capability within your Agent's configuration. This crucial registration
              allows your Agent to discover the tool's existence, understand its purpose (from your
              .proto documentation), and intelligently determine when and how to invoke it.
            </p>
            <p className="text-[10px] font-bold text-foreground/40 uppercase font-mono mb-[4px]">
              In your tools.go file:
            </p>
