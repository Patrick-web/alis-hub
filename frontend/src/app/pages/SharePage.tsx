...
function RoleBadge({ role }: { role: string }) {
  switch (role) {
    ...
    default:
      return <span className="text-[10px] font-mono text-foreground/40">{role}</span>;
  }
}
      ...
      {tooltip && (
        <div className="relative group">
          <Icon
            ...
            className="text-foreground/30 text-[12px] cursor-help"
          />
    ...
    <div className="flex items-center gap-[12px] px-[20px] py-[10px] border-b border-border hover:bg-foreground/[2%] transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        {!person.isGroup && person.email && (
          <p className="text-[10px] font-mono text-foreground/40 truncate">{person.email}</p>
        )}
      </div>
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col bg-background">
      ...
      <div className="px-[20px] py-[6px] border-b border-border flex items-center justify-between shrink-0">
        <p className="font-mono font-bold text-[10px] text-foreground/50 uppercase">Sharing</p>
        {data && (
