...
function Btn({
  ...
  const colors: Record<string, string> = {
    ...
    success:
      "bg-[rgba(52,199,89,0.15)] text-success border border-[rgba(52,199,89,0.3)] hover:bg-[rgba(52,199,89,0.25)]",
    error:
      "bg-[rgba(212,24,61,0.15)] text-destructive border border-[rgba(212,24,61,0.3)] hover:bg-[rgba(212,24,61,0.25)]",
    warning:
      "bg-[rgba(250,200,0,0.12)] text-warning border border-[rgba(250,200,0,0.3)] hover:bg-[rgba(250,200,0,0.22)]",
    info: "bg-[rgba(59,130,246,0.15)] text-info border border-[rgba(59,130,246,0.3)] hover:bg-[rgba(59,130,246,0.25)]",
    ghost:
      "bg-transparent text-foreground/40 border border-border hover:text-foreground hover:border-border",
...
export function NotificationsDebugPage() {
  const { state, addNotification, updateNotification, markAllRead, clearAll, unreadCount } =
    useNotifications();
  ...
  function fireLoadingSuccess() {
    ...
    setTimeout(
      () => notify.success("Done!", { id, description: "The operation completed successfully." }),
      2000,
    );
  }
  ...
  function fireLoadingError() {
    const id = notify.loading("Processing…");
    setTimeout(
      () => notify.error("Failed", { id, description: "Something went wrong. Please try again." }),
      2000,
    );
  }

  function firePromise() {
    notify.promise(new Promise<string>((res) => setTimeout(() => res("payload"), 2500)), {
        ...
        ? [
            {
              label: "Primary",
              variant: "primary",
              onClick: () => notify.success("Primary clicked"),
            },
    ...
    <div className="flex flex-col gap-[32px] p-[28px] max-w-[800px]">
      ...
      <div className="flex items-start justify-between">
        <div className="flex flex-col gap-[4px]">
          <div className="flex items-center gap-[10px]">
            ...
            <h1 className="text-[16px] font-bold text-foreground font-mono">Notifications Debug</h1>
          </div>
      ...
      <Section title="Toast System" icon="solar:notification-lines-remove-linear">
        <Row>
          <Btn
            ...
            variant="success"
            onClick={() =>
              notify.success("Operation succeeded", { description: "Everything went as expected." })
            }
          />
          <Btn
            label="error"
            ...
            variant="error"
            onClick={() =>
              notify.error("Something failed", { description: "Check the logs for details." })
            }
          />
          <Btn
            label="warning"
            ...
            variant="warning"
            onClick={() =>
              notify.warning("Proceed with caution", {
          ...
          <Btn
            ...
            variant="info"
            onClick={() => notify.info("Heads up", { description: "Just a friendly notice." })}
          />
          <Btn
            label="loading"
            icon="solar:refresh-linear"
            onClick={() => notify.loading("Working…")}
          />
        </Row>
        <Row>
          <Btn
            label="loading → success (2s)"
            icon="solar:hourglass-linear"
            variant="success"
            onClick={fireLoadingSuccess}
          />
          <Btn
            label="loading → error (2s)"
            icon="solar:hourglass-linear"
            variant="error"
            onClick={fireLoadingError}
          />
          <Btn label="promise (2.5s)" icon="solar:hourglass-line-linear" onClick={firePromise} />
        </Row>
        <Row>
          <Btn
            label="with action + cancel"
            icon="solar:cursor-linear"
            variant="info"
            onClick={fireWithAction}
          />
          <Btn
            label="persistent (no auto-dismiss)"
            icon="solar:pin-linear"
            onClick={() =>
              notify.info("Pinned notification", {
                persistent: true,
      ...
      <Section title="Notification Center" icon="solar:bell-linear">
        ...
        <div className="bg-muted rounded-[8px] border border-border p-[14px] flex flex-col gap-[12px]">
          {/* Severity */}
          <div className="flex items-center gap-[10px]">
            <span className="text-[10px] text-foreground/30 font-mono uppercase tracking-wide w-[70px] shrink-0">
              Severity
            </span>
          ...
          <div className="flex items-center gap-[20px]">
            {[
              ...
              >
                <span
                  className={`relative w-[28px] h-[16px] rounded-full transition-colors ${val ? "bg-brand-fill" : "bg-accent"}`}
                >
                  <span
                    className={`absolute top-[2px] w-[12px] h-[12px] rounded-full bg-white shadow transition-all ${val ? "left-[14px]" : "left-[2px]"}`}
                  />
                </span>
                {label}
              </button>
        ...
        <Row>
          ...
          <Btn
            label="Running task → done (4s)"
            icon="solar:refresh-linear"
            variant="info"
            onClick={pushRunningTask}
          />
        </Row>
        <Row>
          <Btn
            ...
            return (
              <div className="bg-muted rounded-[8px] border border-border overflow-hidden">
                <div className="px-[12px] py-[8px] border-b border-border flex items-center gap-[6px]">
                  <span className="text-[9px] text-foreground/30 font-mono uppercase tracking-widest">
                    Store state — grouped by source
                  </span>
                    ...
                    return (
                      <div key={source}>
                        <div className="px-[12px] py-[5px] bg-foreground/[0.02] flex items-center gap-[8px]">
                          <span className="text-[9px] font-bold font-mono text-foreground/40 uppercase tracking-wide flex-1">
                            {source}
                          </span>
                          {unread > 0 && (
                            <span className="text-[8px] font-mono text-brand bg-brand-fill/10 px-[5px] py-[1px] rounded-[3px]">
                              {unread} new
                            </span>
                          )}
                        </div>
      ...
      <Section title="System Notifications (macOS)" icon="solar:monitor-linear">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-foreground/35 font-mono mt-[2px]">
                Fires a native notification via UserNotifications. Also togglable in Profile →
            ...
            >
              <span
                className={`absolute top-[2px] w-[14px] h-[14px] rounded-full bg-white shadow transition-all ${sysEnabled ? "left-[16px]" : "left-[2px]"}`}
              />
            </button>
          </div>
        ...
        <Row>
          <Btn
            ...
            disabled={!sysEnabled}
            onClick={() =>
              systemNotify("AlisHub", "This is a test system notification from the debug page.")
            }
          />
          <Btn
            ...
            onClick={() =>
              systemNotify(
                "Build Complete",
                "organisations/voyage/products/vp — build #142 finished in 3m 27s. All checks passed.",
              )
            }
          />
        </Row>
