...
function buildDiff(baseline: models.CodeblockFolder[], newContent: models.NeuronFileContents): DiffEntry[] {
  ...
  (['build', 'infra', 'proto'] as const).forEach(category => {
    ...
    for (const path of allPaths) {
      ...
      entries.push({ category, path, status, oldContent: oldContent ?? '', newContent: newContentForPath ?? '' });
    }
  });
  return entries;
}
...
export function CodeblockUpdatePage() {
  ...
  useEffect(() => {
    ...
    setReviewError(null);
    Promise.all([
      (ProductService.ListCodeblockVersions as (id: string) => Promise<models.CodeblockVersion[]>)(blockId)
        .catch(() => [] as models.CodeblockVersion[]),
      (ProductService.ReadNeuronFileContents as (
        pkg: string, files: models.ScannedNeuronFile[]
      ) => Promise<models.NeuronFileContents | null>)(selectedNeuron.package, scannedFiles),
    ])
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-row bg-background">
        ...
        <div className="flex-1 overflow-auto p-[16px] flex flex-col gap-[20px]">
          <div>
            ...
            <div className="flex flex-col gap-[8px]">
              <StepIndicator index={1} label="Pick Source" active={step === 'source'} done={step === 'review' || step === 'publish'} />
              <StepIndicator index={2} label="Review Changes" active={step === 'review'} done={step === 'publish'} />
              <StepIndicator index={3} label="Publish Version" active={step === 'publish'} done={false} />
            </div>
          </div>
          ...
          {selectedNeuron && (
            <div className="bg-card border border-border rounded-[4px] p-[12px]">
              ...
              <p className="text-[10px] text-foreground/60 font-mono break-all">{selectedNeuron.package}</p>
              <p className="text-[10px] text-foreground/30 mt-[4px]">{selectedFileCount} file(s) selected</p>
            </div>
          )}
          ...
          {step !== 'source' && (
            <div className="bg-card border border-border rounded-[4px] p-[12px]">
              ...
              <p className="text-[10px] text-foreground/60">
                {reviewLoading ? 'Checking…' : hasBaseline ? 'Previous version content available — diff shown' : 'No previous content on record — full replace'}
              </p>
            </div>
          )}
        </div>
        ...
        {step === 'publish' && (
          <div className="p-[10px] border-t border-border flex flex-col gap-[8px]">
            ...
            <Button
              variant="primary"
              className="w-full"
              icon={<Icon icon={publishing ? 'solar:spinner-linear' : 'solar:upload-linear'} className={publishing ? 'animate-spin' : ''} />}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {step === 'source' && (
          <SourceStep
            orgs={orgs} orgsLoading={orgsLoading} orgsError={orgsError} onRetryOrgs={() => setOrgsLoadAttempt(n => n + 1)}
            products={products} productsLoading={productsLoading}
            neurons={neurons} neuronsLoading={neuronsLoading}
            selectedOrg={selectedOrg} setSelectedOrg={setSelectedOrg}
            selectedProduct={selectedProduct} setSelectedProduct={setSelectedProduct}
            selectedNeuron={selectedNeuron} setSelectedNeuron={setSelectedNeuron}
            scannedFiles={scannedFiles} setScannedFiles={setScannedFiles}
        ...
        {step === 'publish' && (
          <div className="flex-1 overflow-auto p-[24px]">
            ...
            <p className="text-[11px] text-foreground/40">Fill in the release details in the sidebar, then click Publish Version.</p>
          </div>
        )}
      </div>

      {openDiffEntry && (
        <DiffModal entry={openDiffEntry} onClose={() => setOpenDiffEntry(null)} />
      )}
    </div>
  );
}
...
function StepIndicator({ index, label, active, done }: {
...
function SourceStep({
  orgs, orgsLoading, orgsError, onRetryOrgs,
  ...
  orgs: InstallOrg[]; orgsLoading: boolean; orgsError: string | null; onRetryOrgs: () => void;
  products: InstallProduct[]; productsLoading: boolean;
  neurons: InstallNeuron[]; neuronsLoading: boolean;
  selectedOrg: string; setSelectedOrg: (v: string) => void;
  selectedProduct: string; setSelectedProduct: (v: string) => void;
  selectedNeuron: InstallNeuron | null; setSelectedNeuron: (n: InstallNeuron | null) => void;
  scannedFiles: models.ScannedNeuronFile[]; setScannedFiles: (fn: (prev: models.ScannedNeuronFile[]) => models.ScannedNeuronFile[]) => void;
  scanLoading: boolean; scanError: string | null;
  canReview: boolean; onReview: () => void;
}) {
  const selectedFileCount = scannedFiles.filter(f => f.selected).length;
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="px-[24px] py-[16px] border-b border-border flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-foreground/40 mt-[2px]">Select the local neuron checkout to publish files from</p>
        </div>
        <Button
          variant="primary"
      ...
      <div className="flex-1 overflow-auto p-[24px] flex flex-col gap-[20px] max-w-[800px]">
        <div className="grid grid-cols-3 gap-[12px]">
          <div>
            ...
            />
            {orgsError && (
              <button onClick={onRetryOrgs} className="mt-[4px] text-[10px] text-destructive hover:underline">
        ...
        {!selectedNeuron ? (
          ...
          <>
            <div className="flex items-center gap-[12px]">
              <button onClick={() => selectAllFiles(true)} className="text-[10px] font-bold uppercase text-foreground/50 hover:text-foreground/80 tracking-wider transition-colors">
                Select All
              </button>
              <span className="text-foreground/20">·</span>
              <button onClick={() => selectAllFiles(false)} className="text-[10px] font-bold uppercase text-foreground/50 hover:text-foreground/80 tracking-wider transition-colors">
                Deselect All
              </button>
            ...
            {(Object.keys(CATEGORY_LABEL) as Category[]).map(cat => {
              const catFiles = scannedFiles.map((f, idx) => ({ ...f, idx })).filter(f => f.category === cat);
              if (catFiles.length === 0) return null;
              return (
                <div key={cat}>
                  <p className="text-[10px] font-bold uppercase text-foreground/40 mb-[8px] tracking-wider">{CATEGORY_LABEL[cat]}</p>
                  <div className="border border-border rounded-[4px] overflow-hidden">
                    {catFiles.map((file, i) => (
                      <label key={file.idx} className={`flex items-center gap-[10px] px-[12px] py-[8px] cursor-pointer hover:bg-foreground/[3%] transition-colors ${i > 0 ? 'border-t border-border' : ''}`}>
                        <input type="checkbox" checked={file.selected} onChange={() => toggleFile(file.idx)} className="accent-brand shrink-0" />
                        <Icon icon="solar:file-code-linear" className="text-foreground/30 shrink-0 text-sm" />
                        <span className={`text-[11px] font-mono truncate ${file.selected ? 'text-foreground/80' : 'text-foreground/30'}`}>{file.path}</span>
                      </label>
                    ))}
                  </div>
                </div>
...
function ReviewStep({
  loading, error, hasBaseline, diffEntries, statusCounts, scannedFiles, onOpenDiff, onContinue,
}: {
  loading: boolean;
  error: string | null;
  hasBaseline: boolean;
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="px-[24px] py-[16px] border-b border-border flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-foreground/40 mt-[2px]">
            {hasBaseline ? 'Compared against the current published version' : 'No previous file content on record for this block'}
          </p>
        </div>
        <Button variant="primary" icon={<Icon icon="solar:alt-arrow-right-linear" />} onClick={onContinue} disabled={loading}>
          Continue to Publish
        </Button>
      </div>
      ...
      <div className="flex-1 overflow-auto p-[24px]">
        {loading ? (
          ...
          <div className="text-[12px] text-destructive bg-[rgba(255,107,107,0.06)] border border-[rgba(255,107,107,0.2)] rounded-[4px] p-[12px]">{error}</div>
        ) : !hasBaseline ? (
          <div className="flex flex-col gap-[16px] max-w-[800px]">
            <div className="text-[12px] text-foreground/60 bg-foreground/[3%] border border-border rounded-[4px] p-[12px]">
              No previous file content on record for this block — this Update will publish a full snapshot of the selected files.
            </div>
            {(Object.keys(CATEGORY_LABEL) as Category[]).map(cat => {
              ...
              return (
                <div key={cat}>
                  ...
                  <div className="border border-border rounded-[4px] overflow-hidden">
                    {files.map((f, i) => (
                      <div key={f.path} className={`px-[12px] py-[8px] text-[11px] font-mono text-foreground/70 ${i > 0 ? 'border-t border-border' : ''}`}>{f.path}</div>
                    ))}
                  </div>
                </div>
              );
                ...
                <div key={cat}>
                  ...
                  <div className="border border-border rounded-[4px] overflow-hidden">
                    {entries.map((e, i) => (
                      ...
                      >
                        <span className={`text-[9px] font-bold uppercase rounded px-[6px] py-[2px] shrink-0 ${STATUS_COLOR[e.status]}`}>
                          {STATUS_LABEL[e.status]}
                        </span>
                        <span className="text-[11px] font-mono truncate text-foreground/70">{e.path}</span>
                      </button>
                    ))}
                  </div>
                </div>
...
function DiffModal({ entry, onClose }: { entry: DiffEntry; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-muted/95 backdrop-blur-sm" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="flex items-center justify-between px-[20px] py-[12px] border-b border-border shrink-0">
        <div className="flex items-center gap-[10px]">
          ...
          <span className={`text-[9px] font-bold uppercase rounded px-[6px] py-[2px] ${STATUS_COLOR[entry.status]}`}>{STATUS_LABEL[entry.status]}</span>
        </div>
        <button onClick={onClose} className="flex items-center gap-[6px] text-[11px] text-foreground/50 hover:text-foreground/80 transition-colors border border-foreground/15 hover:border-foreground/30 rounded px-[10px] py-[4px]">
          <Icon icon="solar:close-circle-linear" className="text-xs" />
          Close
        </button>
      </div>
      <div className="flex-1 overflow-auto">
        <DiffView diffFile={diffFile} diffViewTheme="dark" diffViewMode={DiffModeEnum.Unified} diffViewHighlight diffViewFontSize={11} />
      </div>
    </div>
  );
}
