  ...
  steps: [
      ...
      body: (
        <div className="flex flex-col gap-[12px]">
          <p className="text-[12px] text-foreground/70 leading-[1.7]">
            Alis (pronounced like "Alice") is a development platform that wraps
            Google Cloud Platform and gRPC into a single, opinionated workflow.
            Instead of manually creating GCP projects, configuring Cloud Run,
            setting up IAM roles, and wiring up API gateways, Alis handles all
            of that for you — consistently and repeatably.
          </p>
          <p className="text-[12px] text-foreground/70 leading-[1.7]">
            The core idea is{" "}
            ...
            : you define your service using Protocol Buffers, and Alis manages
            everything needed to get that service running on GCP.
          </p>
          <p className="text-[12px] text-foreground/70 leading-[1.7]">
            Think of Alis as the difference between manually assembling a PC
            from parts versus using a well-engineered workstation — you still
            write the code that matters, but the infrastructure scaffolding is
            handled for you.
          </p>
        </div>
      ),
          ...
          <div className="flex flex-col gap-[8px]">
            {[
              ...
              >
                ...
                <p className="text-[11px] text-foreground/60 leading-[1.5] mb-[6px]">
                  {def}
                </p>
    ...
    },
    {
      ...
      body: (
        <div className="flex flex-col gap-[12px]">
          <p className="text-[12px] text-foreground/70 leading-[1.7]">
            Each product has multiple{" "}
            <span className="text-foreground">environments</span> — isolated
            deployment targets that map directly to GCP resources. The three
          ...
          <p className="text-[11px] text-foreground/45 leading-[1.5]">
            When you select an environment in the top-right dropdown, all build
            and deploy commands in the hub target that environment's GCP
            resources.
          </p>
        </div>
      ),
    },
    {
      ...
      body: (
        <div className="flex flex-col gap-[12px]">
          <p className="text-[12px] text-foreground/70 leading-[1.7]">
            You interact with Alis primarily through its CLI tool and this hub
            app. The three core commands map to the three phases every service
            goes through:
          </p>
          <div className="flex flex-col gap-[6px]">
            {[
              ...
              >
                <p className="text-[12px] font-bold text-brand font-mono mb-[4px]">
                  {cmd}
                </p>
                <p className="text-[11px] text-foreground/55 leading-[1.4]">
                  {desc}
                </p>
              </div>
            ))}
          ...
          <p className="text-[11px] text-foreground/40 leading-[1.5]">
            The following modules walk through each of these in detail, showing
