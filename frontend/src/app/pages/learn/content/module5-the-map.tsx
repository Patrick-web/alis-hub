            ...
            {[
              {
                stage: "Define",
                cmd: "Alis define",
                desc: "Write a .proto file. Alis generate compiles it into Go interfaces and client stubs.",
              },
              {
                stage: "Implement",
                cmd: "(your code)",
                desc: "Fill in the Go server interface with your business logic. The compiler enforces the contract.",
              },
              {
                stage: "Build",
                cmd: "Alis build",
                desc: "Cloud Build compiles and containerises your service. The image lands in Artifact Registry.",
              },
              {
                stage: "Deploy",
                cmd: "Alis deploy",
                desc: "Cloud Run, Endpoints, and IAM are provisioned. Your service gets a live HTTPS endpoint.",
              },
            ...
            {[
              {
                ...
                detail:
                  "The client uses the generated stub (or makes an HTTP call). The request is sent to the Cloud Run HTTPS endpoint.",
              },
              {
                n: "2",
                title: "ESP intercepts",
                detail:
                  "The Endpoints Service Proxy (sidecar) receives the request first. It validates the JWT bearer token against Google's key servers.",
              },
              {
                n: "3",
                title: "Transcoding (if REST)",
                detail:
                  "If the request is REST/JSON, ESP transcodes it to gRPC (matching the proto method) before passing it to your service.",
              },
              {
                n: "4",
                title: "Your handler runs",
                detail:
                  "Your Go implementation receives a typed protobuf request. It runs your business logic, queries databases, calls other services.",
              },
              {
                n: "5",
                title: "Response returned",
                detail:
                  "Your handler returns a proto message. ESP seriAlises it (to JSON for REST, to protobuf for gRPC) and sends it back to the client.",
              },
            ].map(({ n, title, detail }) => (
              <div
                key={n}
              ...
              >
                <span className="text-[9px] font-bold text-brand font-mono shrink-0 w-[14px]">
                  {n}
                </span>
                <div>
                  <span className="text-[11px] font-bold text-foreground font-mono">
