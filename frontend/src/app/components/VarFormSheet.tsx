...
export function VarFormSheet({
  ...
  const toggleEnv = (envName: string) => {
    setPropagations((prev) =>
      prev.map((p) => (p.envName === envName ? { ...p, checked: !p.checked } : p)),
    );
  };

  const toggleCustom = (envName: string) => {
    setPropagations((prev) =>
      prev.map((p) => (p.envName === envName ? { ...p, useCustom: !p.useCustom } : p)),
    );
  };

  const setCustomValue = (envName: string, val: string) => {
    setPropagations((prev) =>
      prev.map((p) => (p.envName === envName ? { ...p, customValue: val } : p)),
    );
  };
  ...
  const handleSubmit = async () => {
    ...
    if (mode === "create" && !LABEL_REGEX.test(trimmed)) {
      setError(
        "Label must start with a letter and contain only letters, digits, hyphens, or underscores (3–101 characters).",
      );
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      await onSubmit(
        trimmed,
        value,
        selectedPropagations.length > 0 ? selectedPropagations : undefined,
      );
      onOpenChange(false);
    } catch (err) {
      ...
      >
        ...
        <div className="flex flex-col gap-[16px] px-[20px] py-[20px] flex-1 overflow-y-auto">
          <div className="flex flex-col gap-[6px]">
            <p className="font-mono text-[10px] font-bold text-foreground/50 uppercase">Label</p>
            <Input
            ...
            {mode === "create" && label && !LABEL_REGEX.test(label.trim()) && (
              <p className="font-mono text-[10px] text-destructive">
                Must start with a letter; letters, digits, <span className="opacity-70">-</span> and{" "}
                <span className="opacity-70">_</span> only; 3–101 chars total.
              </p>
            )}
          </div>

          <div className="flex flex-col gap-[6px]">
            <p className="font-mono text-[10px] font-bold text-foreground/50 uppercase">Value</p>
            <textarea
              placeholder="value"
              value={value}
                ...
                <div className="border-t border-border flex flex-col gap-0">
                  {propagations.map((p) => (
                    ...
                    >
                      ...
                      >
                        <div
                          className={`w-[15px] h-[15px] rounded-[3px] border shrink-0 flex items-center justify-center transition-colors ${
        ...
        <SheetFooter className="px-[20px] py-[14px] border-t border-border flex-row gap-[8px]">
          ...
          <Button
            ...
            disabled={
              loading || !label.trim() || (mode === "create" && !LABEL_REGEX.test(label.trim()))
            }
            icon={
              loading ? (
