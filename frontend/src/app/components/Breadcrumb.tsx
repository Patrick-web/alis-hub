...
export function Breadcrumb({ items }: BreadcrumbProps) {
  return (
    <div className="flex-[1_0_0] h-[30px] min-h-px min-w-px relative">
      <div
        aria-hidden="true"
        className="absolute border-border border-b border-r border-solid border-t inset-0 pointer-events-none"
      />
        ...
        <div className="content-stretch flex items-center px-[10px] relative size-full space-x-1">
          ...
          {items.map((item, index) => (
            <div key={index} className="flex items-center">
              ...
              <p
                className={`font-mono leading-[normal] not-italic relative shrink-0 text-[${index === 0 ? "11px" : "12px"}] text-foreground whitespace-nowrap ${
