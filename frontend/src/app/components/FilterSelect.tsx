...
export function FilterSelect({
  ...
  return (
    <div ref={ref} className={`relative ${isLg ? "w-full" : "shrink-0"}`}>
      ...
      >
        <span className={isPlaceholder ? "text-foreground/30" : ""}>
          {displayLabel || placeholder}
        </span>
        {loading ? (
          <Icon
            icon="solar:refresh-linear"
            className={`${isLg ? "text-[11px]" : "text-[8px]"} animate-spin opacity-50`}
          />
        ) : (
          <Icon
      ...
      {open && (
        <div
          className={`absolute top-full left-0 bg-background border border-border z-50 min-w-full shadow-[0_8px_24px_rgba(0,0,0,0.5)] max-h-[240px] overflow-y-auto ${isLg ? "mt-[2px] rounded-[4px]" : "mt-[3px] rounded-[3px]"}`}
        >
          {loading ? (
            <div
              className={`px-[10px] py-[8px] text-foreground/30 font-mono ${isLg ? "text-[12px]" : "text-[9px] uppercase"}`}
            >
              Loading…
            </div>
          ) : options.length === 0 ? (
              ...
              <button
                ...
                className={`w-full text-left border-b border-border last:border-0 font-mono transition-colors ${
                  isLg
                    ...
                    : "px-[10px] py-[6px] text-[9px] uppercase"
                } ${
                  opt.value === value
