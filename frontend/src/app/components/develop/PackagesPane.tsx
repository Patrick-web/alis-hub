...
export function PackagesPane({ tabId, neuron, neuronNames }: PackagesPaneProps) {
  ...
  async function scan() {
    patchSession<PackagesSession>(tabId, { step: 'scan', scripts: [], selectedScripts: [], error: '' });
    try {
      const allScripts: PackageScript[] = [];
      for (const name of neuronNames) {
        const parsed = parseNeuron(name);
        const result = await PackageService.PreparePackageScripts(orgRef.current, productRef.current, parsed.id, parsed.version, devSettings.ignoreHiddenFolders, devSettings.ignoredFolderPatterns);
  ...
  async function doRunScripts(withVenv: boolean) {
    ...
    for (const script of toRun) {
      const cmd = current.action === 'upgrade_defined' ? script.upgradeDefined
        : current.action === 'upgrade' ? script.upgrade
        : current.action === 'install' ? script.install
        : script.add;
      if (!cmd) continue;
    ...
    if (newSessions.length > 0) {
      ...
      patchSession<PackagesSession>(tabId, { error: 'Failed to start any package scripts', step: 'select-folders' });
    }
  }
  ...
  async function handleRunPackages() {
    ...
    if (hasPython) {
      ...
      if (!venvExists) { patchSession<PackagesSession>(tabId, { step: 'venv-setup' }); return; }
    }
  ...
  return (
    <>
        ...
        <div className="flex-1 overflow-y-auto px-[16px] py-[16px]">
          ...
          <div className="flex flex-col gap-[2px] mb-[20px]">
            {PACKAGE_ACTIONS.map(({ value, label, desc }) => (
              ...
              >
                <span className={`size-[6px] rounded-full shrink-0 ${session.action === value ? 'bg-brand-fill' : 'bg-accent'}`} />
          ...
          <Button
            ...
            onClick={() => session.scripts.length === 1 ? handleRunPackages() : patch({ step: 'select-folders' })}
          >
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <div className="flex-1 overflow-y-auto">
            <div className="px-[16px] pt-[14px] pb-[8px]">
              <button onClick={() => patch({ step: 'select-action' })} className="flex items-center gap-[6px] text-[10px] text-foreground/40 hover:text-foreground mb-[12px] transition-colors">
          ...
          <div className="shrink-0 px-[14px] py-[10px] border-t border-border">
            <Button variant="primary" className="w-full justify-center py-[10px]" disabled={session.selectedScripts.length === 0} onClick={handleRunPackages}>
              Run · {session.selectedScripts.length} folder{session.selectedScripts.length !== 1 ? 's' : ''}
            </Button>
          </div>
        </div>
      )}
      ...
      {session.step === 'venv-setup' && (
        <div className="flex-1 flex flex-col min-h-0 overflow-y-auto px-[16px] py-[16px]">
          ...
          <div className="flex flex-col gap-[8px]">
            ...
            <button onClick={() => patch({ step: 'select-folders' })} className="w-full py-[9px] text-[10px] text-foreground/30 hover:text-foreground transition-colors font-mono">
              ← Back
            </button>
          </div>
        </div>
      ...
      {session.step === 'running' && (
        <PackageRunStatusList sessions={packageSessions} footerText="Output in the terminal pane ↓" />
      )}
    </>
  );
}
