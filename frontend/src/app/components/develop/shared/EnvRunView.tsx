            ...
            >
              {run.done && run.error ? (
                <Icon
                  ...
                  className="text-destructive text-[11px] shrink-0"
                />
              ) : run.done ? (
                <Icon
                  icon="solar:check-circle-linear"
                  className="text-success text-[11px] shrink-0"
                />
              ) : (
                <Loader size={10} />
              )}
      ...
      {runs.map((run) => (
        ...
        >
            ...
            <div className="shrink-0 flex items-center gap-[10px] px-[14px] py-[10px] border-b border-border">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[11px] font-bold text-foreground leading-tight">
                  {runningLabel ?? (planOnly ? "Planning" : "Deploying")}
                  {run.version || fallbackVersion ? ` · ${run.version || fallbackVersion}` : ""}
                </p>
              ...
              <button
                onClick={() => copyLog(run.env)}
                className="shrink-0 text-foreground/30 hover:text-foreground transition-colors"
                title="Copy log"
              >
                <Icon
                  icon={copiedEnv === run.env ? "solar:check-circle-linear" : "solar:copy-linear"}
                  className="text-sm"
                />
              </button>
            </div>
          ...
          {(step === "result" || run.done) && (
            <div
              className={`shrink-0 px-[14px] py-[10px] border-b border-border ${run.error ? "bg-[rgba(255,92,95,0.05)]" : "bg-[rgba(52,199,89,0.05)]"}`}
            >
              {run.error ? (
                <div className="flex items-start gap-[8px]">
                  ...
                  />
                  <p className="text-[10px] text-foreground/70 leading-relaxed flex-1">
                    {run.error}
                  </p>
                  <button
                    onClick={() => copyLog(run.env)}
                  ...
                  >
                    <Icon
                      icon={
                        copiedEnv === run.env ? "solar:check-circle-linear" : "solar:copy-linear"
                      }
                      className="text-sm"
                    />
                  </button>
                ...
                <div className="flex items-center gap-[8px]">
                  ...
                  <div className="min-w-0">
                    <p className="text-[11px] font-bold text-foreground leading-tight">
                      {planOnly ? "Plan Complete" : "Deploy Complete"}
                    </p>
                  ...
                  <div className="ml-auto flex items-center gap-[10px] shrink-0">
                    ...
                    {run.logsUrl && (
                      <button
                        onClick={() => Browser.OpenURL(run.logsUrl)}
