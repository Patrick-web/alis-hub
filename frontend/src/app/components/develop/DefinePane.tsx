...
export function DefinePane({ tabId, neuron, restore }: DefinePaneProps) {
    ...
    if (restore?.task) {
      ...
      patchSession<DefineSession>(tabId, {
        ...
        ...(savedStep === 'running' && meta.operationName
          ? { defineResult: { operationName: meta.operationName as string, definition: '', version: '', notes: '', definitionArtifacts: [], done: false } }
          : {}),
      });
  ...
  async function loadCommits() {
    ...
    try {
      ...
      patchSession<DefineSession>(tabId, { commits: result as DefineCommit[], commitsLoading: false });
    } catch {
  ...
  async function handleRunDefine() {
    ...
    patchSession<DefineSession>(tabId, { step: 'running', defineError: null, progressMsg: 'Starting Define...' });
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (result?.done) {
          ...
          if (!result.error) {
            patchSession<DefineSession>(tabId, { step: 'glass', glassLoading: true, progressMsg: 'Define complete — loading Glass...' });
            ...
            try {
              ...
              patchSession<DefineSession>(tabId, { glassResult: glass as GlassResult, glassLoading: false });
            } catch {
      ...
      {session.step === 'confirm' && session.selectedCommit && (
        <div className="flex-1 overflow-y-auto px-[16px] py-[20px]">
          ...
          <div className="bg-card border border-border rounded-[8px] p-[16px] mb-[20px]">
            ...
            <p className="text-[11px] text-foreground leading-[1.5] mb-[10px]">{session.selectedCommit.message}</p>
            <span className="text-[10px] font-bold font-mono text-brand">{session.selectedCommit.sha.substring(0, 12)}</span>
            <p className="text-[9px] text-foreground/40 mt-[4px]">{session.selectedCommit.author} · {formatTimestamp(session.selectedCommit.timestamp)}</p>
          </div>
