...
const GCLOUD_TOOLS: { id: ToolTab; title: string; keywords: string[]; icon: React.ComponentType<{ className?: string }> }[] = [
  { id: 'buckets',          title: 'Open Buckets',          keywords: ['buckets', 'storage', 'gcs'],              icon: BoxIcon },
  { id: 'logs',             title: 'Open Logs',              keywords: ['logs', 'logging', 'cloud logs'],          icon: FileTextIcon },
  { id: 'artifactregistry', title: 'Open Artifact Registry', keywords: ['artifacts', 'registry', 'docker', 'gar'], icon: DatabaseIcon },
  { id: 'secrets',          title: 'Open Secret Manager',    keywords: ['secrets', 'secret manager', 'gsm'],       icon: KeyRoundIcon },
  { id: 'spanner',          title: 'Open Spanner',           keywords: ['spanner', 'database', 'sql'],             icon: TableIcon },
];
...
export function GCloudCommandsExtension() {
  ...
  useEffect(() => {
    ...
    GS.CheckGCloudStatus().then(setGcloudStatus).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    const commands: CommandItem[] = GCLOUD_TOOLS.map(tool => ({
      ...
      badge: notAuthed
        ? { text: 'Not authenticated', variant: 'warning' as const }
        : undefined,
      onSelect: (ctx) => {
