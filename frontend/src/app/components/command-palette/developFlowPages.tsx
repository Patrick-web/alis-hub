  ...
  switch (type) {
    ...
    case "deploy": {
      ...
      if (step === "loading") ctx.push(deployVersionsPage(tabId, neuron, navigate));
        ...
        if (getSession<DeploySession>(tabId)?.version)
    ...
    case "packages": {
      ...
      if (step === "scan" || step === "select-action")
        ctx.push(packagesActionPage(tabId, neuron, navigate));
      ...
      const items: PaletteListItem[] = [
        {
          ...
          title:
            selected.length > 0
              ? `Continue · ${selected.length} service${selected.length !== 1 ? "s" : ""}`
              : "Continue",
...
function CommitSummaryHeader({ tabId }: { tabId: string }) {
  const session = useDevelopSessions((s) => s.sessions[tabId]) as
    DefineSession | BuildSession | undefined;
  const c =
    session && (session.kind === "define" || session.kind === "build")
      ? session.selectedCommit
      : null;
  if (!c) return null;
...
export function defineCommitsPage(
  tabId: string,
  neuron: string,
  navigate: NavigateFunction,
): PalettePage {
  return {
    ...
    useItems: () => {
      ...
      return {
        items,
        loading: session?.commitsLoading ?? true,
        empty: "No commits found in define repo.",
      };
    },
  };
}

export function defineConfirmPage(
  tabId: string,
  neuron: string,
  navigate: NavigateFunction,
): PalettePage {
...
export function defineProgressPage(
  tabId: string,
  neuron: string,
  navigate: NavigateFunction,
): PalettePage {
...
export function buildCommitsPage(
  tabId: string,
  neuron: string,
  navigate: NavigateFunction,
): PalettePage {
  return {
    ...
    useItems: () => {
      ...
      return {
        items,
        loading: session?.commitsLoading ?? true,
        empty: "No commits found for this branch.",
      };
    },
  };
}
        ...
        ...(session?.deployEnvs ?? []).map((env) => ({
          ...
          onSelect: () => {
            ...
            patchSession<BuildSession>(tabId, {
              selectedDeployEnvs: cur.includes(env.name)
                ? cur.filter((e) => e !== env.name)
                : [...cur, env.name],
            });
          },
        })),
...
export function deployVersionsPage(
  tabId: string,
  neuron: string,
  navigate: NavigateFunction,
): PalettePage {
  return {
    ...
    useItems: () => {
      ...
      return {
        items,
        loading: (session?.step ?? "loading") === "loading",
        empty: "No built versions found.",
      };
    },
  };
}

export function deployEnvsPage(
  tabId: string,
  neuron: string,
  navigate: NavigateFunction,
): PalettePage {
    ...
    useItems: () => {
      ...
      const items: PaletteListItem[] = [
        ...
        }),
        {
          ...
          onSelect: () =>
            patchSession<DeploySession>(tabId, {
              planOnly: !getSession<DeploySession>(tabId)?.planOnly,
            }),
        },
        {
          id: "toggle-beta",
          ...
          keywords: ["beta"],
          onSelect: () =>
            patchSession<DeploySession>(tabId, { beta: !getSession<DeploySession>(tabId)?.beta }),
        },
      ];
      return {
        items,
...
function DeployProtectedConfirm({
  tabId,
  neuron,
  labels,
  navigate,
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-xs leading-relaxed">
            {labels.join(", ")} {labels.length > 1 ? "are" : "is"} protected. Type the phrase below
            to confirm this deploy.
          </p>
        </div>
      </div>
      <p className="bg-accent text-accent-foreground rounded px-2 py-1.5 font-mono text-xs">
        {requireText}
      </p>
      <input
        autoFocus
...
function deployProtectedPage(
  ...
  return {
    ...
    title: "Confirm",
    Component: () => (
      <DeployProtectedConfirm tabId={tabId} neuron={neuron} labels={labels} navigate={navigate} />
    ),
  };
}
...
export function packagesActionPage(
  tabId: string,
  neuron: string,
  navigate: NavigateFunction,
): PalettePage {
...
export function packagesFoldersPage(
  tabId: string,
  neuron: string,
  navigate: NavigateFunction,
): PalettePage {
        ...
        ...(session?.scripts ?? []).map((s) => ({
          ...
          onSelect: () => {
            ...
            patchSession<PackagesSession>(tabId, {
              selectedScripts: cur.includes(s.workDir)
                ? cur.filter((w) => w !== s.workDir)
                : [...cur, s.workDir],
            });
          },
        })),
  ...
  return (
    <div className="border-b px-4 py-3">
      ...
      <p className="text-muted-foreground mt-1 text-[11px] leading-relaxed">
        A <code className="text-brand font-mono">.venv</code> is required before running Python
        package scripts.
      </p>
    </div>
  );
...
export function packagesVenvPage(
  tabId: string,
  neuron: string,
  navigate: NavigateFunction,
): PalettePage {
...
function PackagesRunningView({ tabId }: { tabId: string }) {
  ...
  if (!session || session.step === "preparing")
    return <CenteredLoader label="Starting scripts..." />;
  return (
    <div className="flex flex-col overflow-y-auto">
      <PackageRunStatusList
        sessions={packageSessions}
        footerText="Output shows in the Develop terminal pane"
      />
    </div>
  );
}

export function packagesRunningPage(
  tabId: string,
  neuron: string,
  navigate: NavigateFunction,
): PalettePage {
