...
const NAV_COMMANDS: NavCommand[] = [
  ...
  },
  {
    ...
    icon: HammerIcon,
    path: "/develop",
    keywords: ["develop", "services", "neurons"],
  },
  {
    id: "nav-builds",
    title: "Go to Builds",
  ...
  },
  {
    ...
    icon: GitBranchIcon,
    path: "/git",
    keywords: ["git", "source", "control", "vcs"],
  },
  {
    id: "nav-buildkit",
    ...
    icon: BlocksIcon,
    path: "/buildkit",
    keywords: ["buildkit", "agents", "mcp"],
  },
  {
    id: "nav-codeblocks",
    title: "Go to Codeblocks",
    icon: FileCodeIcon,
    path: "/codeblocks",
    keywords: ["codeblocks", "code"],
  },
];

// ── ResultPanel — generic post-action view ────────────────────────────────────

function ResultPanel({ result, onDismiss }: { result: CommandResult; onDismiss: () => void }) {
  return (
...
function CommandBadge({ text, variant }: { text: string; variant: "warning" | "error" | "info" }) {
  ...
  return (
    <span
      className={`ml-auto flex items-center gap-1 rounded px-1.5 py-0.5 text-[11px] font-medium ${colors[variant]}`}
    >
...
function CommandRow({
  ...
  return (
    ...
    >
      ...
      <span>{cmd.title}</span>
      {cmd.subtitle && <span className="text-muted-foreground ml-1 text-xs">{cmd.subtitle}</span>}
...
function PaletteListPageView({
  ...
  const visible = useMemo(() => {
    ...
    const matched = rest
      .map((item) => ({
        item,
        score: commandScore(item.title, q, [
          ...(item.keywords ?? []),
...
function FooterHint({ keys, label }: { keys: string; label: string }) {
  return (
    <span className="text-muted-foreground flex items-center gap-1.5 text-[11px]">
      {label}
      <kbd className="bg-accent text-accent-foreground rounded px-1 py-0.5 font-mono text-[10px]">
        {keys}
      </kbd>
    </span>
  );
...
export function CommandPalette() {
  const { isOpen, close, extensions, pages, push, replace, pop, popToRoot } = useCommandPalette();
  ...
  const results = useMemo(() => {
    ...
    return allCommands
      ...
      .sort(
        (a, b) =>
          b.score - a.score ||
          a.cmd.groupOrder - b.cmd.groupOrder ||
          a.cmd.title.localeCompare(b.cmd.title),
      )
      .map(({ cmd }) => cmd);
  }, [allCommands, query]);
      ...
      <PalettePageProvider value={ctx}>
        <div className="flex flex-col" onKeyDown={handleRootKeyDown}>
            ...
            <div className="flex items-center gap-1 border-b px-3 py-2">
              ...
              {pages.map((p, i) => (
                <span key={`${i}-${p.id}`} className="flex items-center gap-1">
                  ...
                  {i === pages.length - 1 ? (
                    <span className="text-foreground bg-accent rounded px-1.5 py-0.5 text-[11px] font-medium">
                      {p.title}
                    </span>
                  ) : (
                    <button
                      onClick={() => {
                        for (let n = pages.length - 1; n > i; n--) pop();
                      }}
                      className="text-muted-foreground hover:text-foreground bg-accent/50 rounded px-1.5 py-0.5 text-[11px] transition-colors"
                    >
                      {p.title}
          ...
          {!activePage ? (
            ...
            <div
              key={activePage.id}
              className="flex max-h-[440px] min-h-[200px] flex-col overflow-y-auto"
            >
