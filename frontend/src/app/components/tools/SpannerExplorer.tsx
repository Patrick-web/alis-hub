...
export function SpannerExplorer({ projectID }: Props) {
  ...
  function addQueryTab() {
    ...
    setTabs((prev) => [...prev, id]);
    setQueryTabStates((prev) => ({ ...prev, [id]: { ...DEFAULT_QUERY_TAB_STATE } }));
    activateTab(id);
  }
  ...
  function removeTab(tabId: string) {
    ...
    if (editingTabId === tabId) { setEditingTabId(null); setEditingName(''); }
    setTabs((prev) => {
    ...
    if (isQueryTab(tabId)) {
      setQueryTabStates((prev) => { const n = { ...prev }; delete n[tabId]; return n; });
      setQueryTabNames((prev) => { const n = { ...prev }; delete n[tabId]; return n; });
    } else {
  ...
  function closeOtherTabs(keepTabId: string) {
    ...
    const toKeep = new Set([keepTabId, ...(isQueryTab(keepTabId) ? [] : [firstQueryTab])]);
    ...
    if (qRemove.length > 0) {
      setQueryTabStates((prev) => { const n = { ...prev }; qRemove.forEach((t) => delete n[t]); return n; });
      setQueryTabNames((prev) => { const n = { ...prev }; qRemove.forEach((t) => delete n[t]); return n; });
    }
    if (tRemove.length > 0) setTableTabs((prev) => { const n = { ...prev }; tRemove.forEach((t) => delete n[t]); return n; });
  ...
  function closeAllTabs() {
    ...
    setTableTabs({});
    setQueryTabStates((prev) => ({ [firstQueryTab]: prev[firstQueryTab] ?? { ...DEFAULT_QUERY_TAB_STATE } }));
  ...
  function openTabContextMenu(e: React.MouseEvent, tabId: string) {
    ...
    if (canClose) items.push({ label: 'Close', onClick: () => { setTabContextMenu(null); removeTab(tabId); } });
    if (hasOthers) {
  ...
  function updateTabQuery(tabId: string, update: Partial<QueryTabState>) {
    setQueryTabStates((prev) => ({ ...prev, [tabId]: { ...(prev[tabId] ?? DEFAULT_QUERY_TAB_STATE), ...update } }));
  }
  ...
  function executeQuery(tabId: string) {
    ...
    updateTabQuery(tabId, { queryLoading: true, queryResult: null, dmlResult: null, queryError: null });

    if (isDMLSQL(sqlTrimmed)) {
      GS.ExecuteSpannerDML(selectedDatabase, sqlTrimmed)
        .then((r) => updateTabQuery(tabId, { dmlResult: { rowsAffected: Number(r?.rowsAffected ?? 0) } }))
      ...
      updateTabQuery(tabId, { queryError: 'DDL statements (DROP, TRUNCATE, ALTER) are not supported via this tool.', queryLoading: false });
    } else {
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <div className="flex items-stretch border-b border-[#464646] shrink-0 overflow-x-auto bg-[#1a1a1a]">
          {tabs.map((tabId) => {
            ...
            const label = isQTab ? (queryTabNames[tabId] ?? queryTabLabel(tabId)) : (tableTabs[tabId]?.tableName ?? tabId);
            const icon = isQTab ? 'solar:code-square-linear' : 'hugeicons:table';
            ...
            return (
              <div
                ...
                style={{ minHeight: 36 }}
              >
                <Icon icon={icon} className={`text-xs shrink-0 ${isActive && !isQTab ? 'text-[#f881a9]' : ''}`} />
                {isQTab && editingTabId === tabId ? (
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') { e.preventDefault(); commitRename(tabId); }
                      if (e.key === 'Escape') { setEditingTabId(null); setEditingName(''); }
                      e.stopPropagation();
                    }}
                    onClick={(e) => e.stopPropagation()}
                    className="w-[90px] bg-transparent outline-none text-white text-[10px] font-['JetBrains_Mono',sans-serif] border-b border-[rgba(248,129,169,0.6)]"
                  />
                ) : (
                  <span
                    className="max-w-[110px] truncate"
                    onDoubleClick={isQTab ? (e) => { e.stopPropagation(); startRename(tabId); } : undefined}
                  >
                    {label}
                  </span>
                )}
          ...
          <button
            onClick={addQueryTab}
            className="flex items-center justify-center w-[32px] shrink-0 border-l border-[#2a2a2a] text-[rgba(255,255,255,0.5)] hover:text-white hover:bg-[rgba(255,255,255,0.06)] transition-colors self-stretch"
            title="New query tab"
          >
        ...
        <div className="flex-1 overflow-hidden relative">
          ...
          {tabs.filter(isQueryTab).map((tabId) => {
            ...
            const { sql, queryLoading, queryError, queryResult, dmlResult } = tabQuery;
            return (
              <div key={tabId} className={`absolute inset-0 flex flex-col overflow-hidden ${activeTab === tabId ? '' : 'hidden'}`}>
                  ...
                  <>
                    {!selectedDatabase ? (
                      ...
                      <>
                        ...
                        <div className="flex flex-col shrink-0" style={{ height: queryPanelHeight }}>
                          <div className="px-[16px] pt-[12px] pb-[8px] flex-1 min-h-0 flex flex-col">
                            <div className="flex-1 min-h-0 border border-[#464646] rounded-[4px] overflow-hidden focus-within:border-[rgba(248,129,169,0.4)] transition-colors">
                              <SqlEditor
                                value={sql}
                          ...
                          <div
                            ...
                            className="h-[4px] cursor-row-resize shrink-0 border-t border-[#464646] hover:bg-[rgba(248,129,169,0.3)] transition-colors"
                          />
                        </div>
          ...
          {/* Table tabs */}
          {tabs.filter((t) => !isQueryTab(t)).map((tabId) => {
      ...
      <AlertDialog open={destructivePendingTab !== null} onOpenChange={(open) => { if (!open) setDestructivePendingTab(null); }}>
