...
export function SpannerTableView({
  tableName,
  dbName,
  org,
  product,
  ...
  function loadSpannerSchemaTypes() {
    ...
    PD.GetSpannerColumnProtoTypes(org, product, tableName)
      ...
      .catch(() => {
        /* no spanner.tf mapping available — fall back to native metadata / manual pick */
      });
  }

  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    loadSpannerSchemaTypes();
  }, [protoDecodeEnabled, org, product, tableName]);
  ...
  function openProtoDecodedView(col: string, cellValue: string) {
    ...
    PD.DecodeProtoBytes(org, cellValue, messageType)
      ...
      .catch((e: unknown) =>
        setProtoDecodeView({ col, loading: false, json: null, error: String(e) }),
      );
  }
  ...
  function openFieldQuery(col: string, messageType: string) {
    ...
    setFieldQuery({
      col,
      path: [],
      pathLabels: [messageType.split(".").pop() ?? messageType],
      typeStack: [messageType],
    });
  ...
  function buildColumnMenu(col: string): ContextItem[] {
    ...
    return [
      {
        label: `SELECT ${c} FROM ${t}`,
        onClick: () => navigate(`SELECT ${c}\nFROM ${t}\nLIMIT 50`),
      },
      {
        label: `SELECT DISTINCT ${c}`,
        onClick: () => navigate(`SELECT DISTINCT ${c}\nFROM ${t}\nLIMIT 50`),
      },
      {
        label: `COUNT(*) GROUP BY ${c}`,
        onClick: () =>
          navigate(
      ...
      {
        label: `ORDER BY ${c} ASC`,
        onClick: () => navigate(`SELECT *\nFROM ${t}\nORDER BY ${c} ASC\nLIMIT 50`),
      },
      "divider",
      {
        label: `DELETE WHERE ${c} = ?`,
        destructive: true,
        onClick: () => navigateDestructive(`DELETE FROM ${t}\nWHERE ${c} = ?`),
      },
      {
        label: `UPDATE SET ${c} = ? WHERE ...`,
        destructive: true,
        onClick: () => navigateDestructive(`UPDATE ${t}\nSET ${c} = ?\nWHERE ...`),
      },
    ];
  }
  ...
  function buildCellMenu(col: string, val: string): ContextItem[] {
    ...
    items.push(
      ...
      {
        label: `SELECT WHERE ${col} IS NULL`,
        onClick: () => navigate(`SELECT *\nFROM ${t}\nWHERE ${c} IS NULL\nLIMIT 50`),
      },
      "divider",
      {
        label: `DELETE WHERE ${col} = ${val}`,
        destructive: true,
        onClick: () => navigateDestructive(`DELETE FROM ${t}\nWHERE ${c} = ${v}`),
      },
      {
        label: `UPDATE SET ? WHERE ${col} = ${val}`,
        destructive: true,
        onClick: () => navigateDestructive(`UPDATE ${t}\nSET ? = ?\nWHERE ${c} = ${v}`),
      },
      "divider",
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="border-b border-border px-[14px] py-[10px] shrink-0 space-y-[8px] bg-background">
        ...
        <div className="flex items-start gap-[10px]">
          ...
          <div className="relative shrink-0" ref={colPickerRef}>
            ...
            >
              ...
              <Icon
                icon={showColPicker ? "solar:alt-arrow-up-linear" : "solar:alt-arrow-down-linear"}
                className="text-[9px] text-foreground/35"
              />
            </button>
            ...
            {showColPicker && (
              <div className="absolute z-20 top-[calc(100%+4px)] left-0 bg-muted border border-border rounded-[4px] shadow-xl min-w-[160px]">
                <div className="flex items-center justify-between px-[10px] py-[6px] border-b border-border">
                  <span className="text-[8px] uppercase text-foreground/30 font-mono font-bold">
                    Columns
                  </span>
                  <button
                ...
                <div className="max-h-[220px] overflow-y-auto">
                  {columns.map((col) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-[11px] h-[11px] border rounded-[2px] flex items-center justify-center shrink-0 transition-colors ${checked ? "bg-brand-fill border-brand-fill" : "border-border"}`}
                        >
                          {checked && (
                            <Icon
          ...
          {protoDecodeEnabled && (
            ...
            >
              <Icon
                ...
                className={`text-[11px] ${protoTypesLoading ? "animate-spin" : ""}`}
              />
            ...
            {whereConditions.map((cond) => (
              <div key={cond.id} className="flex items-center gap-[5px]">
                <Icon
                  ...
                  className="text-[10px] text-foreground/25 shrink-0"
                />
                <SearchableSelect
                  value={cond.column}
        ...
        <div className="flex items-center gap-[10px] flex-wrap">
          <div className="flex items-center gap-[5px]">
            <span className="text-[8px] uppercase text-foreground/35 font-mono font-bold">
              Order
            </span>
            <FilterSelect
              value={orderBy}
            ...
            <thead className="sticky top-0 z-10">
              <tr className="bg-muted border-b border-border">
                {result.columns.map((col, colIdx) => {
                  ...
                  const isDecodable =
                    colType === "BYTES" || colType === "PROTO" || !!tfColumnProtoTypes[col];
                      ...
                      {protoDecodeEnabled && isDecodable && (
                        <div className="mt-[4px] normal-case" onClick={(e) => e.stopPropagation()}>
                          <SearchableSelect
                            ...
                            options={protoTypeOptions.map((t) => ({
                              label: t.fullName,
                              value: t.fullName,
                            }))}
                            onChange={(val) =>
                              setProtoColumnTypes((prev) => ({ ...prev, [col]: val }))
                            }
      ...
      {result && !loading && result.columns.length > 0 && (
        <div className="flex items-center justify-between px-[14px] py-[7px] border-t border-border shrink-0 bg-background">
          ...
          <div className="flex items-center gap-[6px]">
            ...
            <span className="text-[9px] text-foreground/35 font-mono">p{page + 1}</span>
            <button
              onClick={() => changePage(page + 1)}
              disabled={!hasNext}
              className="flex items-center gap-[3px] h-[22px] px-[8px] text-[9px] font-mono border border-border rounded-[3px] text-foreground disabled:opacity-30 hover:enabled:border-foreground/30 transition-colors disabled:cursor-not-allowed"
        ...
        <AlertDialogContent className="text-foreground">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-foreground/50 text-[10px] font-mono">
              This will prefill a destructive SQL statement in the Query tab. Review carefully
              before running.
          ...
          <div className="flex-1 overflow-y-auto">
            {selectedRow &&
              columns.map((col, i) => (
                <div
                  key={col}
                  className="flex items-start gap-[8px] px-[14px] py-[8px] border-b border-border last:border-0 group"
                >
                  <span className="text-[8px] uppercase text-foreground/30 font-mono shrink-0 w-[100px] pt-[2px] truncate">
                    {col}
                  </span>
                  <span
                    className={`text-[10px] font-mono flex-1 break-all ${selectedRow[i] === "NULL" ? "text-foreground/20 italic" : "text-foreground/85"}`}
                  >
                    {selectedRow[i]}
                  </span>
                  <button
      ...
      <Sheet
        open={!!protoDecodeView}
        onOpenChange={(open) => {
          if (!open) setProtoDecodeView(null);
        }}
      >
        <SheetContent
          side="right"
        ...
        >
          ...
          {protoDecodeView?.json && (
            <div className="px-[14px] py-[10px] border-t border-border">
              <button
                onClick={() =>
                  protoDecodeView.json && navigator.clipboard.writeText(protoDecodeView.json)
                }
                className="flex items-center gap-[5px] px-[10px] py-[5px] bg-muted border border-border rounded-[3px] text-[9px] font-mono text-foreground/60 hover:text-foreground hover:border-foreground/30 transition-colors uppercase"
              >
                <Icon icon="solar:copy-linear" className="text-[11px]" />
        ...
        >
                  ...
                  <span key={i} className="flex items-center gap-[3px]">
                    {i > 0 && (
                      <Icon
                        ...
                        className="text-[8px] text-foreground/25"
                      />
                    )}
                    <button
                      onClick={() => goBackTo(i)}
          ...
          <div className="flex-1 overflow-y-auto">
            {fieldQueryLoading ? (
              ...
              <p className="p-[14px] text-[10px] text-red-400 font-mono whitespace-pre-wrap">
                {fieldQueryError}
              </p>
            ) : fieldQueryPicked ? (
              <div className="p-[14px] flex flex-col gap-[10px]">
                ...
                <div className="flex items-center gap-[6px]">
                  <span className="text-[8px] uppercase text-foreground/35 font-mono font-bold shrink-0">
                    Op
                  </span>
                  <SearchableSelect
                    value={fieldQueryOp}
                ...
                {!NO_VALUE_OPS.includes(fieldQueryOp) && (
                  <div className="flex items-center gap-[6px]">
                    ...
                    {fieldQueryPicked.kind === "enum" ? (
                      ...
                      <input
                        type={
                          fieldQueryPicked.scalarType === "int" ||
                          fieldQueryPicked.scalarType === "float"
                            ? "number"
                            : "text"
                        }
                        value={fieldQueryValue}
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="flex items-center gap-[4px] shrink-0">
                        <span className="text-[8px] uppercase text-foreground/30 font-mono">
                          {kindLabel}
                        </span>
