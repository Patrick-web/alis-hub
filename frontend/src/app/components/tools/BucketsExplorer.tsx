    ...
    >
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-full gap-[6px] px-[12px]">
          ...
          <p className="text-[8px] text-foreground/30 font-mono text-center leading-tight">
            {error}
          </p>
        </div>
      ) : kind === "image" && content ? (
...
export function BucketsExplorer({ projectID }: Props) {
  ...
  const selectedContentType =
    selectedObject?.contentType || objectMeta?.contentType || "application/octet-stream";
            ...
            buckets.map((b) => (
              ...
              >
                <Icon
                  ...
                  className={`text-base shrink-0 ${selectedBucket === b.name ? "text-brand" : "text-foreground/40"}`}
                />
                <span
                  className={`text-[10px] font-mono truncate ${selectedBucket === b.name ? "text-foreground" : "text-foreground/60"}`}
                >
                  {b.name}
                </span>
              </button>
              ...
              <>
                ...
                {objects.map((obj) => (
                  <button
                    key={obj.name}
                    onClick={() =>
                      setSelectedObject((prev) => (prev?.name === obj.name ? null : obj))
                    }
                    onDoubleClick={() => {
                  ...
                  >
                    <Icon
                      ...
                      className={`text-sm shrink-0 ${selectedObject?.name === obj.name ? "text-brand" : "text-foreground/30"}`}
                    />
                    <span
                      className={`text-[10px] font-mono flex-1 truncate ${selectedObject?.name === obj.name ? "text-foreground" : "text-foreground/70"}`}
                    >
                  ...
                  <button
                    onClick={() =>
                      GS.OpenInConsole(
                        "storage-object",
                        projectID,
                        `${selectedBucket}/${selectedObject.name}`,
                      )
                    }
