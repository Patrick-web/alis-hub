...
function buildFilter(
  ...
  if (text.trim())
    parts.push(`(textPayload:"${text.trim()}" OR jsonPayload.message:"${text.trim()}")`);
  return parts.join(" ");
}
...
export function LogsExplorer({ projectID }: Props) {
  ...
  const load = useCallback(
    (append = false, serviceOverride?: string) => {
      let filter: string;
  ...
  const pollStream = useCallback(() => {
    ...
    GS.ListLogEntries(projectID, filter, "")
      .then((page) => {
        setError(null);
        const incoming = (page.entries ?? []).filter(
          (e) => !seenIdsRef.current.has(e.insertId ?? ""),
        );
  ...
  useEffect(() => {
    pollStreamRef.current = pollStream;
  }, [pollStream]);
        ...
        <>
          ...
          <div className="flex items-center gap-[8px] px-[16px] py-[10px] border-b border-border flex-wrap">
            ...
            <Button
              ...
              icon={
                <Icon
                  icon={isStreaming ? "solar:pause-circle-linear" : "solar:play-circle-linear"}
                  className="text-xs"
                />
              }
            >
                ...
                {!hasLoaded && !loading && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-[8px]">
                    <Icon
                      ...
                      className="text-4xl text-foreground/10"
                    />
                ...
                {virtualItems.map((virtualRow) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span className="text-[9px] text-foreground/30 shrink-0 mt-[1px] w-[70px]">
                          {ts}
                        </span>
                        <span
                          className={`text-[8px] uppercase px-[5px] py-[1px] rounded-[2px] shrink-0 ${SEVERITY_STYLES[sev] ?? SEVERITY_STYLES["DEFAULT"]}`}
                        >
                          {sev.slice(0, 4)}
                        </span>
