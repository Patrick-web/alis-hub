...
> = {
  ...
  index_lock: {
    ...
    text: "text-amber-400",
    label:
      "Git is locked by a stale index.lock file, likely left behind by a crashed git process. Clear the lock to continue.",
  },
  other_error: {
...
export function GitOperationBanner({
    ...
    switch (result.kind) {
      case "up_to_date":
        return onDismiss ? (
          <button
            onClick={onDismiss}
            className="text-[10px] px-[8px] py-[2px] rounded-[3px] border border-blue-400/30 hover:border-blue-400/60 text-blue-400 transition-colors"
          >
            Dismiss
          </button>
        ) : null;
      case "push_rejected":
        return onSync ? (
          <button
            onClick={onSync}
            className="text-[10px] px-[8px] py-[2px] rounded-[3px] border border-amber-400/30 hover:border-amber-400/60 text-amber-400 transition-colors"
          >
            Pull and Push
          </button>
        ) : null;
  ...
  const displayMessage =
    result.kind === "auth_error" && signInError
      ...
      : result.kind === "index_lock" && clearLockError
        ...
        : result.kind === "other_error"
          ? expanded
            ? result.message
            : result.message.slice(0, 120) + (result.message.length > 120 ? "…" : "")
          : label;
  ...
  return (
    <div className={`shrink-0 px-3 py-1.5 ${cfg.bg} border-b ${cfg.border} flex items-start gap-2`}>
      ...
      <span
        className={`text-xs ${cfg.text} flex-1 leading-[1.5] ${result.kind === "other_error" ? "font-mono whitespace-pre-wrap break-all" : ""}`}
      >
        {displayMessage}
      </span>
