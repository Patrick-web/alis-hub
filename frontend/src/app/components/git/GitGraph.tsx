...
type ListItem =
  { kind: "commit"; graphRow: GraphRow } | { kind: "file"; commitHash: string; file: CommitFile };
...
export function GitGraph({
  ...
  const unpushedHashes = useMemo((): Set<string> => {
    ...
    const currentBranch = headCommit.refNames
        ...
        {virtualizer.getVirtualItems().map((vItem) => {
          ...
          if (item.kind === "file") {
            const isFileSelected =
              selectedCommitFile?.hash === item.commitHash &&
              selectedCommitFile?.path === item.file.path;
            return (
              <div
              ...
              >
                <span
                  className={`text-[10px] font-bold w-3 shrink-0 ${fileStatusColor(item.file.statusCode)}`}
                >
                ...
                <span
                  className="text-[10px] text-foreground/20 truncate shrink-0 max-w-[40%]"
                  title={item.file.path}
                >
                  {item.file.path.includes("/")
                    ? item.file.path.split("/").slice(0, -1).join("/")
                    : ""}
                </span>
              </div>
            );
                        ...
                        <TooltipTrigger asChild>
                          <span className="shrink-0 flex items-center">
                            <Icon
                              ...
                              className="text-amber-400/70 text-[12px]"
                            />
                          </span>
                        </TooltipTrigger>
