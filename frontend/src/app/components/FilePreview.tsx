...
export function kindIcon(kind: PreviewKind): string {
  switch (kind) {
    ...
    case "video":
      return "solar:videocamera-linear";
    case "audio":
      return "solar:music-note-linear";
    case "text":
      return "solar:code-linear";
    default:
      ...
      <div className="relative flex flex-col bg-muted border border-border rounded-[6px] shadow-2xl w-[90vw] max-w-[1100px] h-[85vh]">
        ...
        <div className="flex items-center justify-between px-[16px] py-[12px] border-b border-border shrink-0">
          <div className="flex items-center gap-[10px] min-w-0">
            <Icon icon={kindIcon(kind)} className="text-base text-foreground/40 shrink-0" />
            <span className="text-[11px] text-foreground font-mono truncate">{filename}</span>
            <span className="text-[9px] text-foreground/30 font-mono shrink-0">{contentType}</span>
          </div>
          <button
            onClick={onClose}
            className="text-foreground/40 hover:text-foreground transition-colors shrink-0 ml-[16px]"
          >
        ...
        <div className="flex-1 overflow-hidden flex items-center justify-center p-[24px]">
          {loading ? (
            <div className="flex flex-col items-center gap-[12px]">
              <Loader size={32} />
              <p className="text-[10px] text-foreground/30 font-mono">Loading preview…</p>
            </div>
          ) : error ? (
            <div className="text-center max-w-[360px]">
              <Icon
                icon="solar:cloud-cross-linear"
                className="text-4xl text-foreground/10 mb-[12px]"
              />
              <p className="text-[11px] text-red-400 font-mono">{error}</p>
            </div>
          ) : kind === "unsupported" ? (
            <div className="text-center">
              <Icon icon="solar:file-linear" className="text-4xl text-foreground/10 mb-[12px]" />
              <p className="text-[11px] text-foreground/40 font-mono">
                No preview available for {contentType}
              </p>
            </div>
          ) : kind === "image" ? (
            <img
            ...
            <iframe
              src={dataURL}
              title={filename}
              className="w-full h-full rounded-[3px] border-0"
            />
          ) : kind === "video" ? (
            ...
            <pre className="w-full h-full overflow-auto text-[11px] text-foreground/80 font-mono leading-relaxed whitespace-pre-wrap break-all">
              {b64ToText(content)}
            </pre>
          ) : null}
        </div>
