  ...
  closeTab: (id) =>
    set((state) => {
      ...
      return {
        tabs: next,
        activeTabId:
          state.activeTabId === id ? (next[next.length - 1]?.id ?? null) : state.activeTabId,
      };
    }),
  closeMultiple: (ids) =>
    set((state) => {
      ...
      return {
        tabs: next,
        activeTabId: activeStillPresent ? state.activeTabId : (next[next.length - 1]?.id ?? null),
      };
    }),
  activateTab: (id) => set({ activeTabId: id }),
  setTabNotificationId: (tabId, notifId) =>
    set((state) => ({
