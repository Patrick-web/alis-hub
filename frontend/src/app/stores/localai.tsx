...
type LocalAIAction =
  ...
  | { type: 'SET_STATUS'; payload: { binaryReady: boolean; ollamaRunning: boolean; modelPulled: boolean } }
...
function reducer(state: LocalAIState, action: LocalAIAction): LocalAIState {
  switch (action.type) {
    ...
    case 'BINARY_DOWNLOAD_START':
      return { ...state, binaryDownloading: true, binaryError: null, binaryDownloadPct: 0, binaryDownloadLabel: '' };
    case 'BINARY_DOWNLOAD_PROGRESS':
      return { ...state, binaryDownloadPct: action.payload.pct, binaryDownloadLabel: action.payload.label };
...
export function LocalAIProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    const status = await LocalAIService.GetOllamaStatus().catch(() => ({ binaryReady: false, running: false }));
  ...
  useEffect(() => {
    const offProg = Events.On('localai:ollama-download-progress', (ev: any) => {
      const d = ev?.data ?? ev;
      dispatch({ type: 'BINARY_DOWNLOAD_PROGRESS', payload: { pct: d?.pct ?? 0, label: d?.label ?? '' } });
    });
